...
For now, only support >= 256 color terminals :)"
  (declare (indent 0))
  `(let ((colorful '((class color) (min-colors 256)))
         ,@(mapcar (lambda (cons)
                     (list (intern (car cons)) (cdr cons)))
                   (append starlit-default-colors
                           starlit-custom-colors)))
     ,@body))
...
(with-starlit-color-vars
  (custom-theme-set-faces
   ...
   `(helm-ff-file-extension ((,colorful (:foreground ,type))))
   ...
   `(org-meta-line ((,colorful (:inherit (fixed-pitch font-lock-comment-face) :foreground ,shadow))))
